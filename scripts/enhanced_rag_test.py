"""Enhanced RAG Pipeline - Instrumented Test Report (9 questions).

Tests the upgraded RAG pipeline (semantic chunking, hybrid search, cross-encoder
reranking) by sending 9 balanced questions (3 SQL, 3 RAG, 3 General) through
the real Streamlit UI, capturing screenshots, and generating a PDF report.

Usage:
    # Start Streamlit first:  streamlit run app.py
    # Then run:
    python scripts/enhanced_rag_test.py [--no-screenshots] [--app-url URL]
"""
...
# ---------------------------------------------------------------------------
# 9 Test Questions (3 SQL, 3 RAG, 3 General)
# ---------------------------------------------------------------------------
...
SUPERVISOR_PROMPT = (
    "You are a query classifier for a customer support system. Your job is to "
    "analyze the user's message and route it to the correct specialist agent.\n\n"
    "You must classify each query into exactly one of these categories:\n\n"
    "1. sql_agent: For queries about customer data, accounts, profiles, support "
    "tickets, products, billing history, subscription details, or any structured "
    "data lookups.\n\n"
    "2. rag_agent: For queries about company policies, terms of service, privacy "
    "policy, refund policy, procedures, guidelines.\n\n"
    "3. general: For greetings, general conversation, clarifications, or queries "
    "that don't fit the above categories."
)

SQL_AGENT_PROMPT = (
    "You are a customer support SQL agent with access to a SQLite database "
    "containing customer support data.\n\n"
    "Database Tables:\n"
    "- customers: customer profiles (name, email, phone, account_type, "
    "subscription_tier, join_date, address, account_status)\n"
    "- products: product catalog (name, category, price, description)\n"
    "- tickets: support tickets (subject, description, category, priority, "
    "status, channel, assigned_agent, created_at, resolved_at, resolution, "
    "satisfaction_rating)"
)

RAG_AGENT_PROMPT = (
    "You are a company policy expert agent. You have access to a retrieval tool "
    "that searches through company policy documents (refund policy, privacy "
    "policy, terms of service).\n\n"
    "Enhanced Pipeline:\n"
    "- Semantic chunking (SemanticChunker with percentile breakpoint)\n"
    "- Hybrid search: BM25 (k=20, weight=0.4) + Dense (k=20, weight=0.6)\n"
    "- Cross-encoder reranking (ms-marco-MiniLM-L-6-v2, top_n=5)"
)

GENERAL_AGENT_PROMPT = (
    "You are a friendly customer support assistant for TechCorp Inc. You handle "
    "general inquiries, greetings, and help guide users to the right resources."
)
...
# ---------------------------------------------------------------------------
# Run single question through graph
# ---------------------------------------------------------------------------
...
def run_single_question(graph, question_text, tracker, thread_id=None):
    ...
    router_calls = all_calls[:1] if all_calls else []
    ...
    total_tokens_val = sum(c["total_tokens"] for c in all_calls)
...
def generate_pdf_report(results, output_dir):
    ...
    class Report(FPDF):
        def header(self):
            ...
            self.cell(
                0,
                7,
                "Enhanced RAG Pipeline - Instrumented Test Report",
                new_x=XPos.LMARGIN,
                new_y=YPos.NEXT,
                align="C",
            )
    ...
    pdf.cell(
        0,
        10,
        "Enhanced RAG Pipeline Test Report",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
        align="C",
    )
    ...
    pdf.cell(
        0,
        6,
        "Semantic Chunking + Hybrid Search (BM25/Dense) + Cross-Encoder Reranking",
        new_x=XPos.LMARGIN,
        new_y=YPos.NEXT,
        align="C",
    )
    ...
    pipeline_lines = [
        "Documents --> SemanticChunker (percentile=90) --> ChromaDB",
        "",
        "Query --> BM25 (k=20) ---|",
        "                         +--> EnsembleRetriever (0.4/0.6)",
        "Query --> Dense (k=20) --|        |",
        "                                  v",
        "                    CrossEncoderReranker (top_n=5) --> RAG Agent",
    ]
    ...
    pdf.body(
        f"Routing Accuracy: {correct}/{len(results)} ({100 * correct / len(results):.0f}%)"
    )
    pdf.ln(1)

    pdf.body("Agent Routing Distribution:")
    ...
    for label, subset in [
        ...
        if subset:
            ...
            pdf.body(
                f"  {label}: avg={avg_t:.1f}s, min={min(times):.1f}s,"
    ...
    for name, prompt in [
        ...
        ("RAG Agent (Enhanced)", RAG_AGENT_PROMPT),
    ...
    pdf_path = output_dir / "Enhanced_RAG_Test_Report.pdf"
...
def main():
    parser = argparse.ArgumentParser(
        description="Enhanced RAG Pipeline Test (9 questions)"
    )
    parser.add_argument(
    ...
    output_dir = PROJECT_ROOT / "test_results_enhanced_rag"
    screenshots_dir = output_dir / "screenshots"
    screenshots_dir.mkdir(parents=True, exist_ok=True)

    print(f"\n{'=' * 70}")
    print("  Enhanced RAG Pipeline - Instrumented Test")
    print("  Semantic Chunking + BM25/Dense Hybrid + Cross-Encoder Reranker")
    print(
        f"  Questions: {len(TEST_QUESTIONS)} | Screenshots: {not args.no_screenshots}"
    )
    ...
    for i, q in enumerate(TEST_QUESTIONS):
        ...
        if not args.no_screenshots:
            ...
            take_ui_screenshot(question, ss_path, app_url=args.app_url)
    ...
    # Save JSON
    json_path = output_dir / "enhanced_rag_results.json"
